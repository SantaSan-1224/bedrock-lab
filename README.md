...
- **Phase 3**: AWS 運用エージェント (自前 tool use ループ + AgentCore Runtime、read-only ツール) — [docs/plan_phase3.md](docs/plan_phase3.md) / 2方式の実測比較は [docs/agent_comparison.md](docs/agent_comparison.md)

## 構成

```
[開発端末: chat.py / mini_rag.py / agent_cli.py (boto3)]
        |  一時クレデンシャル (assume-role)
        v
[Bedrock / ap-northeast-1]
   ├─ Converse API ──────────────→ [Claude (jp.anthropic.*) — 東京/大阪のみ。国外に出ない]
   │     └─ tool use ループ (Phase 3): get_cost / search_logs / list_resources / search_kb
...
[AgentCore Runtime (Phase 3 方式①)]
   └─ 同じエージェントコードを直接コードデプロイ (S3 zip)。実行ロールがツール権限を持ち、
      stdout は CloudWatch (/aws/bedrock-agentcore/...) へ。呼び出しは InvokeAgentRuntime
...
read-only ツール4種 (コスト照会 / ログ調査 / リソース棚卸し / KB 検索) を持つ AWS 運用エージェント。

```bash
# 方式②: ローカル実行 (自前 tool use ループ。--verbose でツール選択が見える)
python agent_cli.py --profile bedrock-lab --verbose

# 方式①: AgentCore Runtime へデプロイ (直接コードデプロイ。Docker/ECR 不要)
scripts/build_runtime_zip.sh          # arm64/py3.13 依存を zip 化
(cd terraform && terraform apply)     # aws_bedrockagentcore_agent_runtime を作成

# 方式①: Runtime 呼び出し (会話文脈は runtimeSessionId 単位、/new で切替)
...
- エージェント分 (Phase 3): AgentCore Runtime は実行秒課金 (vCPU $0.0895/h + メモリ $0.00945/GB-h、I/O 待機中の CPU は無課金)。**Cost Explorer API は $0.01/リクエスト**のため、get_cost ツールの呼び出し頻度に注意 (ループにステップ上限あり)
- 固定費はゼロ。使わなければ課金されない

## Phase 4 以降 (将来)

- **Phase 4 (最適化)**: プロンプトキャッシュ (エージェントはツール定義+システムプロンプトを毎ターン再送するため効果検証に最適)、Guardrails
- **AgentCore の残り要素**: Gateway (ツールの MCP 化) / Memory / Identity
